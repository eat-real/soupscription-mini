...
function AutoFillModal({ menu, week, onFill, onClose, t }) {
  ...
  const buildPlan = (targetKcal, daysCount, targetBudget = null) => {
    ...
    const sb = shuffle(breakfasts), ss = shuffle(soups), sm = shuffle(mains), se = shuffle(extrasPool);
    ...
    for (const day of week) {
      ...
      if (!day.blocked.has("breakfast") && sb.length) {
        ...
        dp.breakfast = mkEntry(d);
        totalSpend += d.price;
      }
      if (ss.length) {
        const d = ss[count % ss.length];
        const sideIds = d.sides || [];
        const autoSides = sideIds.length > 0
          ? [sidesAll.find(s => s.id === sideIds[Math.floor(Math.random() * sideIds.length)])].filter(Boolean)
          : [];
        dp.lunch = mkEntry(d, autoSides);
        totalSpend += d.price + autoSides.reduce((s,sd)=>s+sd.price,0);
      }
      if (sm.length) {
        ...
        const autoSides = sideIds.length > 0
          ? [sidesAll.find(s => s.id === sideIds[Math.floor(Math.random() * sideIds.length)])].filter(Boolean)
          : [];
        dp.dinner = mkEntry(d, autoSides);
        totalSpend += d.price + autoSides.reduce((s,sd)=>s+sd.price,0);
      }

      // Добираем калории / бюджет через extras
      const dayKcal = () => {
        let k = 0;
      ...
      const shouldAdd = () => {
        if (targetBudget) return totalSpend < targetBudget - 2; // добираем до бюджета
        return targetKcal > 1600 && dayKcal() < targetKcal - 200;
      };

      let attempts = 0;
      while (shouldAdd() && attempts < 8 && se.length) {
        const candidate = se[attempts % se.length];
        const alreadyUsed = [dp.breakfast, dp.lunch, dp.dinner, ...(dp.extras||[])]
          .filter(Boolean).map(e => e.dish?.id);
        if (!alreadyUsed.includes(candidate.id)) {
          dp.extras = [...(dp.extras || []), mkEntry(candidate)];
...
export default function App() {
  ...
  const LangToggle = () => (
    <button onClick={() => setLang(l => l === "en" ? "ru" : "en")} style={{
      ...
      padding: "4px 10px", color: "rgba(255,255,255,.85)", fontSize: 12, fontWeight: 600, cursor: "pointer",
