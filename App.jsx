...
async function fetchMenu() {
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEETS_ID}/values/menu!A:L?key=${SHEETS_KEY}`;
...
function slotBlock(dow, slotKey) {
  ...
  if (dow <= 1 || dow === 2) return dow <= 1 ? 0 : 1;
...
function calcDelivery(plan, week) {
  ...
  for (const [date, slots] of Object.entries(plan)) {
    ...
    for (const [sk, dish] of Object.entries(slots)) {
      if (dish) bt[slotBlock(dow, sk)] += dish.price;
    }
  }
...
function formatSummary(plan, week, userName) {
  ...
  for (const day of week) {
    ...
    const filled = Object.entries(dp).filter(([, d]) => d);
    if (!filled.length) continue;
    const kcal  = filled.reduce((s, [, d]) => s + d.kcal, 0);
    const price = filled.reduce((s, [, d]) => s + d.price, 0);
    foodTotal += price;
    const icon = day.isDelivery ? "🚚" : "📅";
    lines.push(`${icon} *${day.name} ${day.num}*`);
    for (const [sk, d] of filled) {
      const slot = SLOTS.find(s => s.key === sk);
      lines.push(`  ${slot?.label}: ${d.name} — ${d.price}€`);
    }
    lines.push(`  ┄ ${kcal} kcal · ${price.toFixed(2)}€\n`);
  }
  const { fee, bt } = calcDelivery(plan, week);
  const BNAMES = ["Mon delivery", "Wed delivery", "Fri delivery"];
    ...
    <div className="page">
      ...
      {items.map((d, i) => (
        <div key={d.id} className="mc fu" style={{ animationDelay: `${i * .05}s` }}>
          <div style={{ height: 130, overflow: "hidden", position: "relative", background: "var(--surf2)" }}>
...
function PlannerPage({ menu, week, plan, setPlan, onCheckout }) {
  ...
  const placeInSlot = (sk) => {
    ...
    setPlan(p => ({ ...p, [selDay]: { ...(p[selDay] || {}), [sk]: selected } }));
    setSelected(null);
  };
  ...
  const hasAny = Object.values(plan).some(dp => Object.values(dp).some(Boolean));
  const { fee } = calcDelivery(plan, week);
  const foodTotal = Object.values(plan).reduce((s, dp) =>
    s + Object.values(dp).reduce((ss, d) => ss + (d?.price || 0), 0), 0);
  ...
  return (
    <div className="page">
      ...
      <div className="day-strip" style={{ marginBottom: 12 }}>
        {week.map(d => {
          const hasDishes = Object.values(plan[d.key] || {}).some(Boolean);
          return (
              ...
              {blocked ? (
                ...
                <>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, lineHeight: 1.3 }}>{entry.name}</div>
                    <div style={{ fontSize: 11, color: "var(--a)", fontWeight: 700, marginTop: 2 }}>{entry.kcal} kcal · {entry.price}€</div>
                  </div>
                ...
                <span style={{ fontSize: 12, color: "var(--ink3)" }}>
                  {selected && selected.fresh && !day?.freshOk ? "🚫 Fresh only on delivery day" : "Select a dish below first"}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="dish-row">
          ...
          {catItems.map(d => (
            <div key={d.id} className={`dish-card${selected?.id === d.id ? " sel" : ""}`}
              ...
              <div className="dish-card-img" style={{ position: "relative" }}>
                ...
                {selected?.id === d.id && <div style={{ position: "absolute", inset: 0, background: "rgba(61,122,53,.2)", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ background: "white", borderRadius: "50%", width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--g)", fontWeight: 900, fontSize: 14 }}>✓</span></div>}
              </div>
...
function SummaryPage({ plan, week, onBack, onConfirm, sending }) {
  const userName = TG_USER ? `${TG_USER.first_name || ""} ${TG_USER.last_name || ""}`.trim() : "Guest";
  const { text, total } = formatSummary(plan, week, userName);
  const { fee, bt } = calcDelivery(plan, week);
  const BNAMES = ["Mon block", "Wed block", "Fri block"];
  ...
  return (
    <div className="page">
      ...
      {week.filter(d => Object.values(plan[d.key] || {}).some(Boolean)).map(day => {
        const dp = plan[day.key] || {};
        const filled = Object.entries(dp).filter(([, d]) => d);
        const kcal = filled.reduce((s, [, d]) => s + d.kcal, 0);
        const price = filled.reduce((s, [, d]) => s + d.price, 0);
            ...
            {filled.map(([sk, d]) => {
              ...
              return (
                <div key={sk} className="sum-slot">
                  ...
                  <span style={{ fontWeight: 600 }}>{d.name} <span style={{ color: "var(--a)" }}>{d.price}€</span></span>
                </div>
              );
            })}
          </div>
      ...
      <div style={{ marginBottom: 14 }}>
        {Object.entries(bt).map(([bi, bv]) => bv > 0 && (
          <div key={bi} className={`del-block ${bv >= FREE_THRESHOLD ? "free" : "paid"}`}>
            <span style={{ fontSize: 12, fontWeight: 600 }}>{BNAMES[bi]}</span>
      ...
      <button className="btn btn-g" onClick={onConfirm} disabled={sending} style={{ marginBottom: 10 }}>
...
export default function App() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (tg) {
        tg.sendData(JSON.stringify({ plan, total }));
      }
