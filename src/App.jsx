...
function AutoFillModal({ menu, week, onFill, onClose }) {
  ...
  const handleFill = () => {
    ...
    const extras     = menu.filter(d => d.cat !== "side" && !d.fresh);
    ...
    for (const day of week) {
      ...
      const dp = {};
      if (!day.blocked.has("breakfast") && sb.length) dp.breakfast = mkEntry(sb[count % sb.length]);
      if (ss.length) dp.lunch   = mkEntry(ss[count % ss.length]);
      if (sm.length) dp.dinner  = mkEntry(sm[count % sm.length]);

      // Добираем калории через extras пока не достигнем цели ±200
      const dayKcal = () => {
        let k = 0;
        if (dp.breakfast) k += dp.breakfast.dish.kcal;
        if (dp.lunch)     k += dp.lunch.dish.kcal;
        if (dp.dinner)    k += dp.dinner.dish.kcal;
...
function PlannerPage({ menu, week, plan, setPlan, onCheckout }) {
  ...
  const catItems = menu.filter(d => d.cat === activeCat && d.cat !== "side" && !(d.fresh && !day?.freshOk));
