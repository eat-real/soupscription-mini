  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 200, display: "flex", alignItems: "flex-end" }}>
      <div style={{ background: "var(--surf)", borderRadius: "18px 18px 0 0", padding: "20px 16px 32px", width: "100%" }} className="fu">
        <div style={{ fontFamily: "Playfair Display,serif", fontSize: 18, marginBottom: 2 }}>🍚 Add sides</div>
        <div style={{ fontSize: 12, color: "var(--ink3)", marginBottom: 14 }}>to {dish.name} · pick as many as you like</div>
        ...
        <button className="btn btn-g" onClick={() => onSelect(selected)} style={{ marginBottom: 8 }}>
          {selected.length > 0 ? `Add ${selected.length} side${selected.length > 1 ? "s" : ""} →` : "No sides →"}
        </button>
  ...
  const handleFill = () => {
    ...
    for (const day of week) {
      ...
      if (!day.blocked.has("breakfast") && sb.length)
        dp.breakfast = mkEntry(sb[count % sb.length]);

      if (ss.length) {
        const lunchDish = ss[count % ss.length];
        // Если у блюда есть гарниры — выбираем случайный
        const lunchSideIds = lunchDish.sides || [];
        const lunchSides = lunchSideIds.length > 0
          ? [sidesAll.find(s => s.id === lunchSideIds[Math.floor(Math.random() * lunchSideIds.length)])].filter(Boolean)
          : [];
        dp.lunch = mkEntry(lunchDish, lunchSides);
      }

      if (sm.length) {
        const dinnerDish = sm[count % sm.length];
        const dinnerSideIds = dinnerDish.sides || [];
        const dinnerSides = dinnerSideIds.length > 0
          ? [sidesAll.find(s => s.id === dinnerSideIds[Math.floor(Math.random() * dinnerSideIds.length)])].filter(Boolean)
          : [];
        dp.dinner = mkEntry(dinnerDish, dinnerSides);
      }

      // Добираем калории через extras
      const dayKcal = () => {
        let k = 0;
        if (dp.breakfast) k += dp.breakfast.dish.kcal + (dp.breakfast.sides||[]).reduce((s,sd)=>s+sd.kcal,0);
      ...
      if (kcal > 1600 && se.length) {
        let attempts = 0;
        while (dayKcal() < kcal - 200 && attempts < 8) {
  ...
  return (
    <div className="page">
      ...
      {sidePicker && <SidePickerModal dish={sidePicker.dish} sides={sidePicker.sides}
        ...
        onSkip={() => commitSlot(sidePicker.slotKey, sidePicker.dish, [])} />}
    </div>
  );
}
