    ...
    return {
      ...
      portions: parseInt(d.portions || 0),
        img_url: (() => {
          ...
          const raw = k ? (d[k] || "") : "";
          if (!raw) return "";
          // Неполный Unsplash ID типа "photo-1517673400267"
  ...
  return (
    <div className="page">
      ...
      {(() => {
        ...
        const kcal = entries.reduce((s, [k, v]) => k === "extras" ? s + (v||[]).reduce((es,d)=>es+d.kcal,0) : s+(v?.kcal||0), 0);
        const p    = entries.reduce((s, [k, v]) => k === "extras" ? s + (v||[]).reduce((es,d)=>es+d.p,0) : s+(v?.p||0), 0);
        const f    = entries.reduce((s, [k, v]) => k === "extras" ? s + (v||[]).reduce((es,d)=>es+d.f,0) : s+(v?.f||0), 0);
        const c    = entries.reduce((s, [k, v]) => k === "extras" ? s + (v||[]).reduce((es,d)=>es+d.c,0) : s+(v?.c||0), 0);
        const price= entries.reduce((s, [k, v]) => k === "extras" ? s + (v||[]).reduce((es,d)=>es+d.price,0) : s+(v?.price||0), 0);
            ...
            {filled.map(([sk, v]) => {
              ...
              return (
                <div key={sk} className="sum-slot">
                  ...
                  <span style={{ fontWeight: 600 }}>{v.name} <span style={{ color: "var(--a)" }}>{v.price}€</span></span>
                </div>
              );
            })}
          </div>
