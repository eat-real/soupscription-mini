...
function calcDelivery(plan, week) {
  ...
  for (const [date, slots] of Object.entries(plan)) {
    ...
    for (const [sk, v] of Object.entries(slots)) {
      if (sk === "extras") {
        (v || []).forEach(d => { bt[dayToBlock(dow)] += d.price; });
      } else if (v) {
        bt[slotBlock(dow, sk)] += v.price;
      }
    }
  }
...
function formatSummary(plan, week, userName) {
  ...
  for (const day of week) {
    ...
    const kcal = filled.reduce((s, [k, v]) => {
      if (k === "extras") return s + (v || []).reduce((es, d) => es + d.kcal, 0);
      return s + (v?.kcal || 0);
    }, 0);
    const price = filled.reduce((s, [k, v]) => {
      if (k === "extras") return s + (v || []).reduce((es, d) => es + d.price, 0);
      return s + (v?.price || 0);
    }, 0);
    foodTotal += price;
    ...
    for (const [sk, v] of filled) {
      if (sk === "extras") {
        (v || []).forEach(d => lines.push(`  ★ Extras: ${d.name} — ${d.price}€`));
      } else {
        const slot = SLOTS.find(s => s.key === sk);
        lines.push(`  ${slot?.label}: ${v.name} — ${v.price}€`);
      }
    }
...
function AutoFillModal({ menu, week, onFill, onClose }) {
  ...
  const handleFill = () => {
    ...
    const sb = shuffle(breakfasts), ss = shuffle(soups), sm = shuffle(mains);
    const newPlan = {};
    let count = 0;
    for (const day of week) {
      if (count >= days) break;
...
function PlannerPage({ menu, week, plan, setPlan, onCheckout }) {
  ...
  const placeInSlot = (sk) => {
    ...
    if (sk === "extras") {
      // Extras: unlimited stack
      setPlan(p => {
        const existing = p[selDay]?.extras || [];
        return { ...p, [selDay]: { ...(p[selDay] || {}), extras: [...existing, selected] } };
      });
    } else {
  ...
  const foodTotal = Object.values(plan).reduce((s, dp) => {
    return s + Object.entries(dp).reduce((ss, [k, v]) => {
      if (k === "extras") return ss + (v || []).reduce((es, d) => es + d.price, 0);
      return ss + (v?.price || 0);
    }, 0);
  }, 0);
      ...
      <div style={{ borderRadius: "var(--r)", border: "1.5px dashed var(--bd)", background: selected ? "var(--gxp)" : "var(--surf)", marginBottom: 8, overflow: "hidden", transition: "all .13s" }}
        ...
        {(dayPlan.extras || []).map((d, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", borderBottom: i < (dayPlan.extras.length - 1) ? "1px solid var(--surf2)" : "none" }}>
            ...
            <div style={{ fontSize: 11, color: "var(--a)", fontWeight: 700 }}>{d.price}€</div>
            <button onClick={e => { e.stopPropagation(); removeExtra(i); }}
